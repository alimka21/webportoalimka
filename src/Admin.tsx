...
export default function Admin() {
  ...
  const ADMIN_EMAILS = ["kpbgalimka@gmail.com", "admin@alimkadigital.com"];
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If user not found and they match the exact credentials asked, we could create them,
      // but standard approach is to let them know if login fails.
      if (error.code === 'auth/user-not-found' && loginEmail === 'admin@alimkadigital.com') {
         try {
           await createUserWithEmailAndPassword(auth, loginEmail, loginPassword);
         } catch (createError: any) {
            Swal.fire('Gagal Login', `Email/Password salah atau belum terdaftar: ${createError.message}`, 'error');
         }
      } else {
        Swal.fire('Gagal Login', `Email atau Password salah. (${error.message})`, 'error');
      }
    }
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface p-4">
        <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-xl max-w-sm w-full text-center border border-outline-variant/10">
          <h1 className="text-2xl font-black text-on-surface mb-2">Login Admin</h1>
          <p className="text-sm text-on-surface-variant mb-6">Silakan login untuk mengelola portofolio.</p>
          
          <button 
            onClick={handleLogin}
            className="w-full bg-surface-container hover:bg-surface-container-high text-on-surface py-3 rounded-xl font-bold transition-colors flex items-center justify-center gap-3 border border-outline-variant/20 mb-6"
          >
          ...
          <div className="relative flex items-center justify-center mb-6">
             ...
             <span className="bg-surface-container-lowest px-4 text-xs font-bold text-on-surface-variant absolute">ATAU</span>
          </div>

          <form onSubmit={handleEmailLogin} className="space-y-4 text-left">
            <div>
               <label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1 px-1">Email Admin</label>
               <input 
                 type="email" 
                 required 
                 value={loginEmail}
                 onChange={e => setLoginEmail(e.target.value)}
                 className="w-full bg-surface-container px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" 
                 placeholder="admin@alimkadigital.com"
               />
            </div>
            <div>
               <label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1 px-1">Password</label>
               <input 
                 type="password" 
                 required 
                 value={loginPassword}
            ...
            <button 
              ...
              className="w-full bg-primary text-on-primary py-3 rounded-xl font-black uppercase tracking-widest text-sm hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 flex items-center justify-center gap-2 mt-2"
            >
              {isLoggingIn ? <Loader2 size={18} className="animate-spin" /> : 'Login Email'}
            </button>
          </form>
        </div>
      </div>
